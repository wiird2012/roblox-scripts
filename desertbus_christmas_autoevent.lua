...
It should start working as long you get close to the gift boxes
...
while _G.jedalfarm == true do
    task.wait(.5) -- Increase the delay if this lags for you
        for i,v in pairs(workspace.Decorations.Jedals.Gifts["Script"]:GetChildren()) do
            if v.Name == "Base" then -- Checks if model name is Base, otherwise it errors as soon it runs because of a new model named "Model". Crazy
